...
UndistorterOpenCV::UndistorterOpenCV(const char* configFileName)
{
	...
	std::getline(infile,l3);
	std::getline(infile,l4);
    parseData(l1,l2,l3,l4);
}

void  UndistorterOpenCV::parseData(std::string l1, std::string l2, std::string l3, std::string l4) {
	...
	originalK_ = cv::Mat(3, 3, CV_64F, cv::Scalar(0));
	originalK_.at<double>(0, 0) = (double) inputCalibration[0];
	originalK_.at<double>(1, 1) = (double) inputCalibration[1];
	originalK_.at<double>(2, 2) = (double) 1;
	originalK_.at<double>(0, 2) = (double) inputCalibration[2];
	originalK_.at<double>(1, 2) = (double) inputCalibration[3];
    ...
    printf( " K: %f,%f,%f ",K_.at<double>(0, 0) ,K_.at<double>(0, 2) ,K_.at<double>(1, 1) ,K_.at<double>(1, 2));
