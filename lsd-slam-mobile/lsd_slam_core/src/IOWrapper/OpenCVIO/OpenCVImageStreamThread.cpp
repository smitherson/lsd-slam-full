/**
* This file is part of LSD-SLAM.
*
* Copyright 2013 Jakob Engel <engelj at in dot tum dot de> (Technical University of Munich)
* For more information see <http://vision.in.tum.de/lsdslam> 
*
* LSD-SLAM is free software: you can redistribute it and/or modify
* it under the terms of the GNU General Public License as published by
* the Free Software Foundation, either version 3 of the License, or
* (at your option) any later version.
*
* LSD-SLAM is distributed in the hope that it will be useful,
* but WITHOUT ANY WARRANTY; without even the implied warranty of
* MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
* GNU General Public License for more details.
*
* You should have received a copy of the GNU General Public License
* along with LSD-SLAM. If not, see <http://www.gnu.org/licenses/>.
*/

#include "OpenCVImageStreamThread.h"
//#include <ros/callback_queue.h>

#include <boost/thread.hpp>
#include <opencv2/highgui/highgui.hpp>

//#include "cv_bridge/cv_bridge.h"
#include "util/settings.h"

#include <iostream>
#include <fstream>

#include "IOWrapper/NotifyBuffer.h"
#include <stdio.h>
#include <time.h>
#include <android/log.h>
#define LOGI(...) ((void)__android_log_print(ANDROID_LOG_INFO, "threaded_app", __VA_ARGS__))
#define LOGE(...) ((void)__android_log_print(ANDROID_LOG_ERROR, "threaded_app", __VA_ARGS__))
namespace lsd_slam
{


using namespace cv;

OpenCVImageStreamThread::OpenCVImageStreamThread(char* pathToStream)
{
	// subscribe
	//vid_channel = nh_.resolveName("image");
	//vid_sub          = nh_.subscribe(vid_channel,1, &OpenCVImageStreamThread::vidCb, this);

    LOGI("Trying to create input stream\n");
	// wait for cam calib
	width_ = height_ = 0;

	// imagebuffer
	imageBuffer = new NotifyBuffer<TimestampedMat>(8);
	undistorter = 0;
	lastSEQ = 0;

	haveCalib = false;
    
    //if (strcmp(pathToStream, "0") != 0 ) {
    //    cap =   VideoCapture(pathToStream); 
    //} else {
    //cap = VideoCapture();
    //cap.open(0); 
    //}
    /*if( !cap.isOpened())  // check if we succeeded
    {
        LOGE("Capture failed to open, exitin!\n");
        exit(1);
    }
    return;
    cap.set(CV_CAP_PROP_FRAME_WIDTH, 640);
    cap.set(CV_CAP_PROP_FRAME_HEIGHT, 480);
    cap.set(CV_CAP_PROP_FPS, 50);*/

}

OpenCVImageStreamThread::~OpenCVImageStreamThread()
{
//	delete imageBuffer;
}

void OpenCVImageStreamThread::setCalibrationFromString(std::string parameters)
{
	if(parameters == "")
	{
		LOGE("No camera calibration!\n");
        exit(0);
	}
	else
	{
		undistorter = new UndistorterOpenCV();
	    std::string l1, l2, l3, l4;
        std::stringstream ss(parameters);

	    std::getline(ss, l1);
	    std::getline(ss, l2);
	    std::getline(ss, l3);
	    std::getline(ss, l4);

        undistorter->parseData(l1,l2,l3,l4);


		if(undistorter==0)
		{
			LOGE("Failed to read camera calibration from file... wrong syntax?\n");
			exit(0);
		}

	    fx_ = undistorter->getK().at<double>(0, 0);
        fy_ = undistorter->getK().at<double>(1, 1);
        cx_ = undistorter->getK().at<double>(2, 0);
        cy_ = undistorter->getK().at<double>(2, 1);

		width_ = undistorter->getOutputWidth();
		height_ = undistorter->getOutputHeight();
    }
	haveCalib = true;
}


void OpenCVImageStreamThread::setCalibration(std::string file)
{
	if(file == "")
	{
		LOGE("No camera calibration!\n");
        exit(0);
	}
	else
	{
		undistorter = Undistorter::getUndistorterForFile(file.c_str());

		if(undistorter==0)
		{
			LOGE("Failed to read camera calibration from file... wrong syntax?\n");
			exit(0);
		}

	    fx_ = undistorter->getK().at<double>(0, 0);
        fy_ = undistorter->getK().at<double>(1, 1);
        cx_ = undistorter->getK().at<double>(2, 0);
        cy_ = undistorter->getK().at<double>(2, 1);

		width_ = undistorter->getOutputWidth();
		height_ = undistorter->getOutputHeight();
    }
	haveCalib = true;
}

void OpenCVImageStreamThread::run()
{
//	boost::thread thread(boost::ref(*this));
}

void OpenCVImageStreamThread::operator()()
{
/*	ros::spin();
*/
	exit(0); 
}

inline  NotifyBuffer<TimestampedMat>* OpenCVImageStreamThread::getBuffer() {

	//cv_bridge::CvImagePtr cv_ptr = cv_bridge::toCvCopy(img, sensor_msgs::image_encodings::MONO8);
    /*Mat img;
    cap >> img; // get a new frame from camera
    if (img.rows == 0) {
        waitKey(0);
    }
    cvtColor(img, img, CV_BGR2GRAY);

	TimestampedMat bufferItem;
    bufferItem.timestamp = Timestamp(time (NULL));//ros::Time::now().toSec());

	if(undistorter != 0)
	{
		assert(undistorter->isValid());
		undistorter->undistort(img,bufferItem.data);
	}
	else
	{
		bufferItem.data = img;
	}

    imshow("Live image: ", bufferItem.data);
	imageBuffer->pushBack(bufferItem);*/
    return imageBuffer;
}


void OpenCVImageStreamThread::vidCb(const cv::Mat imgRGB)
{
	if(!haveCalib) return;
    //LOGI("vidcb1");

    if (imgRGB.rows == 0) {
        //waitKey(0);
        return;
    }
    //LOGI("vidcb1.5");
    Mat img;
    cvtColor(imgRGB, img, CV_BGR2GRAY);
    //LOGI("vidcb2");

	TimestampedMat bufferItem;
	bufferItem.timestamp = Timestamp(time (NULL));
    //LOGI("vidcb3");

	if(undistorter != 0)
	{
        //LOGI("vidcb4");

		assert(undistorter->isValid());
		undistorter->undistort(img,bufferItem.data);
	}
	else
	{
		bufferItem.data = img;
	}
    //LOGI("vidcb4");

    //imshow("Live image: ", bufferItem.data);
	imageBuffer->pushBack(bufferItem);
}

//void OpenCVImageStreamThread::infoCb(const sensor_msgs::CameraInfoConstPtr info)
//{
/*	if(!haveCalib)
	{
		fx_ = info->P[0];
		fy_ = info->P[5];
		cx_ = info->P[2];
		cy_ = info->P[6];

		if(fx_ == 0 || fy_==0)
		{
			LOGE("camera calib from P seems wrong, trying calib from K\n");
			fx_ = info->K[0];
			fy_ = info->K[4];
			cx_ = info->K[2];
			cy_ = info->K[5];
		}

		width_ = info->width;
		height_ = info->height;

		LOGE("Received OpenCV Camera Calibration: fx: %f, fy: %f, cx: %f, cy: %f @ %dx%d\n",fx_,fy_,cx_,cy_,width_,height_);
	}
}*/

}
