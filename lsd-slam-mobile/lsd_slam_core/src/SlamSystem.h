...
public:
    ...
    cv::Mat* getDebugImageDepth();
    void debugImageDepthLock();
    void debugImageDepthUnlock();

private:


	// ============= EXCLUSIVELY TRACKING THREAD (+ init) ===============
	TrackingReference* trackingReference; // tracking reference for current keyframe. only used by tracking. It's a point cloud that is reprojected.
	SE3Tracker* tracker;



	// ============= EXCLUSIVELY MAPPING THREAD (+ init) =============
	DepthMap* map; // * Keeps a detailed depth map (consisting of DepthMapPixelHypothesis) and does stereo comparisons and regularization to update it.
	TrackingReference* mappingTrackingReference; // again a point cloud
