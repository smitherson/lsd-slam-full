...
: SLAMEnabled(enableSLAM), relocalizer(w,h,K)
{
	if(w%16 != 0 || h%16!=0)
	{
		LOGI("image dimensions must be multiples of 16! Please crop your images / video accordingly.\n");
		assert(false);
	}
...
SlamSystem::~SlamSystem()
{
	...
	LOGI("... waiting for SlamSystem's threads to exit\n");
	...
	LOGI("DONE waiting for SlamSystem's threads to exit\n");
...
{
	LOGI("Started mapping thread!\n");
	while(keepRunning)
	{
	//LOGI("Mapping iteration!\n");

		if (!doMappingIteration())
		{
    	    //LOGI("Mapping iteration bad!\n");

            //wait some time to get more frames to map in the map
	...
	LOGI("Exited mapping thread \n");
}

void SlamSystem::finalize()
{
	LOGI("Finalizing Graph... finding final constraints!!\n");
	...
	LOGI("Finalizing Graph... optimizing!!\n");
	...
	LOGI("Finalizing Graph... publishing!!\n");
	...
	usleep(200000);
	LOGI("Done Finalizing Graph.!!\n");
}


void SlamSystem::constraintSearchThreadLoop()
{
	LOGI("Started  constraint search thread!\n");
	...
	while(keepRunning)
	{
		if(newKeyFrames.size() == 0)
		{
			...
			if(!doneSomething)
			{
				if(enablePrintDebugInfo && printConstraintSearchInfo)
					LOGI("nothing to re-track... waiting.\n");
		...
		if(doFullReConstraintTrack)
		{
			...
			LOGI("Done optizing Full Map! Added %d constraints.\n", added);
	...
	LOGI("Exited constraint search thread \n");
}

void SlamSystem::optimizationThreadLoop()
{
	LOGI("Started optimization thread \n");
	...
	while(keepRunning)
	{
		...
		{
			LOGI("doing final optimization iteration!\n");
	...
	LOGI("Exited optimization thread \n");
}
...
{
	if(enablePrintDebugInfo && printThreadingInfo)
		LOGI("DISCARDING KF %d\n", currentKeyFrame->id());

	if(currentKeyFrame->idxInKeyframes >= 0)
	{
		LOGI("WARNING: trying to discard a KF that has already been added to the graph... finalizing instead.\n");
	...
	else
	{
		if(force)
		{
			if(noCreate)
			{
				...
				nextRelocIdx = -1;
				LOGI("mapping is disabled & moved outside of known map. Starting Relocalizer!\n");
			}
			else
...
void SlamSystem::addTimingSamples()
{
	...
	if(sPassed > 1.0f)
	{
		...
		{
			LOGI("MapIt: %3.1fms (%.1fHz); Track: %3.1fms (%.1fHz); Create: %3.1fms (%.1fHz); FindRef: %3.1fms (%.1fHz); PermaTrk: %3.1fms (%.1fHz); Opt: %3.1fms (%.1fHz); FindConst: %3.1fms (%.1fHz);\n",
...
cv::Mat* SlamSystem::getDebugImageDepth() {return &(map->debugImageDepth);}
void SlamSystem::debugImageDepthLock() { map->debugImageDepthlock.lock();}
void SlamSystem::debugImageDepthUnlock() { map->debugImageDepthlock.unlock();}
...
void SlamSystem::takeRelocalizeResult()
{
	...
	if(!tracker->trackingWasGood || tracker->lastGoodCount / (tracker->lastGoodCount + tracker->lastBadCount) < 1-0.75f*(1-MIN_GOODPERGOODBAD_PIXEL))
	{
		if(enablePrintDebugInfo && printRelocalizationInfo)
			LOGI("RELOCALIZATION FAILED BADLY! discarding result.\n");
...
bool SlamSystem::doMappingIteration()
{
	...
	if(!doMapping && currentKeyFrame->idxInKeyframes < 0)
	{
		...
		map->invalidate();
		LOGI("Finished KF %d as Mapping got disabled!\n",currentKeyFrame->id());
	...
	if(trackingIsGood)
	{
		if(!doMapping)
		{
			//LOGI("tryToChange refframe, lastScore %f!\n", lastTrackingClosenessScore);
...
void SlamSystem::gtDepthInit(uchar* image, float* depth, double timeStamp, int id)
{
	...
	LOGI("Done GT initialization!\n");
}


void SlamSystem::randomInit(uchar* image, double timeStamp, int id)
{
	LOGI("Doing Random initialization!\n");

	if(!doMapping)
		LOGI("WARNING: mapping is disabled, but we just initialized... THIS WILL NOT WORK! Set doMapping to true.\n");
	...
	LOGI("Done Random initialization!\n");
...
void SlamSystem::trackFrame(uchar* image, unsigned int frameID, bool blockUntilMapped, double timestamp)
{
	...
	if(!trackingIsGood) //when we loose track
	{
    ...
    //in case we using a point cloud of an old keyframe      or current keyframe has been updated its depth
	{
		trackingReference->importFrame(currentKeyFrame.get());
		currentKeyFrame->depthHasBeenUpdatedFlag = false;
		trackingReferenceFrameSharedPT = currentKeyFrame;
    ...
    //we take the pose of the current keyfreame
	...
	if (enablePrintDebugInfo && printThreadingInfo)
		LOGI("TRACKING %d on %d\n", trackingNewFrame->id(), trackingReferencePose->frameID);


	poseConsistencyMutex.lock_shared();
    //gets the initial xi for the new frame position, not sure how ??
    ...
    //calculates the rotation and translation for the new frame compared to the current trackingRefernece
    //the trackingReference is the current keyframe pointcloud 
    ...
    //if we failed for some reason we print information
	...
	{
		LOGI("TRACKING LOST for frame %d (%1.2f%% good Points, which is %1.2f%% of available points, %s)!\n",
				trackingNewFrame->id(),
    ...
    //add the frame to the keyframe graph? possibly update the keyframe depth estimate
    //there is other function for adding keyframes to the graph
	...
	if(blockUntilMapped && trackingIsGood)
	{
		...
		while(unmappedTrackedFrames.size() > 0)
		{
			//LOGI("TRACKING IS BLOCKING, waiting for %d frames to finish mapping.\n", (int)unmappedTrackedFrames.size());
			newFrameMappedSignal.wait(lock);
		}
		lock.unlock();
	}
		...
		float strictness)
{
	...
	{
		if(enablePrintDebugInfo && printConstraintSearchInfo)
			LOGI("FAILE %d -> %d (lvl %d): errs (%.1f / %.1f / -).",
				newKFTrackingReference->frameID, candidateTrackingReference->frameID,
				2,
	...
	{
		if(enablePrintDebugInfo && printConstraintSearchInfo)
			LOGI("FAILE %d -> %d (lvl %d): errs (%.1f / %.1f / %.1f).",
					newKFTrackingReference->frameID, candidateTrackingReference->frameID,
					1,
...
int SlamSystem::findConstraintsForNewKeyFrames(Frame* newKeyFrame, bool forceParent, bool useFABMAP, float closeCandidatesTH)
{
	...
	for(std::unordered_set<Frame*>::iterator c = candidates.begin(); c != candidates.end();)
	{
		...
		{
			if(enablePrintDebugInfo && printConstraintSearchInfo)
				LOGI("SKIPPING %d on %d cause it already exists as constraint.\n", (*c)->id(), newKeyFrame->id());
	...
	for(std::unordered_set<Frame*>::iterator c = closeCandidates.begin(); c != closeCandidates.end();)
	{
		...
		{
			if(enablePrintDebugInfo && printConstraintSearchInfo)
				LOGI("SKIPPING %d on %d (NEAR), cause we already have tried it.\n", (*c)->id(), newKeyFrame->id());
	...
	for(unsigned int i=0;i<farCandidates.size();i++)
	{
		...
		{
			if(enablePrintDebugInfo && printConstraintSearchInfo)
				LOGI("SKIPPING %d on %d (FAR), cause we already have tried it.\n", farCandidates[i]->id(), newKeyFrame->id());
	...
	if (enablePrintDebugInfo && printConstraintSearchInfo)
		LOGI("Final Loop-Closure Candidates: %d / %d close (%d failed, %d inconsistent) + %d / %d far (%d failed, %d inconsistent) = %d\n",
	...
	for (Frame* candidate : closeCandidates)
	{
		...
		if(e1 != 0)
		{
			...
			for(unsigned int k=0;k<farCandidates.size();k++)
			{
				...
				{
					if(enablePrintDebugInfo && printConstraintSearchInfo)
						LOGI(" DELETED %d from far, as close was successful!\n", candidate->id());
	...
	if(parent != 0 && forceParent)
	{
		...
		else
		{
			...
			LOGI("warning: reciprocal tracking on new frame failed badly, added odometry edge (Hacky).\n");
...
bool SlamSystem::optimizationIteration(int itsPerTry, float minChange)
{
	...
	if(enablePrintDebugInfo && printOptimizationInfo)
		LOGI("did %d optimization iterations. Max Pose Parameter Change: %f; avgChange: %f. %s\n", its, maxChange, sumChange / sum,
