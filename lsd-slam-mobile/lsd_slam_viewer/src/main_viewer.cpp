...
#include <boost_utils/portable_binary_iarchive.hpp>
...
void dataReader( int argc, char** argv ) {
    ...
    std::cout << "Server ready, waiting for client." << std::endl;
    acceptor.accept(*s.rdbuf()); // 2
    cout << "Accepted client.\n";
    portable_binary_iarchive ar(s);
    cout << "Starting reading loop.\n";
    for(;;)
    {
