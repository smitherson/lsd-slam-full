...
void PointCloudViewer::addFrameMsg(lsd_slam::keyframeMsgConstPtr msg)
{
	meddleMutex.lock();
...
void PointCloudViewer::addGraphMsg(lsd_slam::keyframeGraphMsgConstPtr msg)
{
	meddleMutex.lock();
...
void PointCloudViewer::draw()
{
	...
	{
		double tm = time(NULL)  - animationPlaybackTime;
	...
	if(saveAllVideo)
	{
		double span = time(NULL) - lastRealSaveTime;
		if(span > 0.4)
		{
			...
			char buf[500];
			snprintf(buf,500,"%s%lf.png",save_folder.c_str(),  time(NULL)); // ros::Time::now().toSec());
			saveSnapshot(QString(buf));
			lastRealSaveTime = time(NULL);// ros::Time::now().toSec();
		}
    ...
    switch (e->key())
    {
      ...
      case Qt::Key_O:
    	  ...
    	  else
    	  {
    		  ...
    		  animationPlaybackTime = time(NULL);// ros::Time::now().toSec();
    	  }
      	  break;
