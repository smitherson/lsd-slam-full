...
#include "NetworkOutput3DWrapper.h"
...
NetworkOutput3DWrapper::NetworkOutput3DWrapper(int width, int height, char* address, char* port)
{	this->width = width;
	this->height = height;
/*
...
*/
    ...
    pub = new Publisher(address, port);
   	publishLvl=0;
	printf("Started QApplication thread\n");

}

NetworkOutput3DWrapper::~NetworkOutput3DWrapper()
{
    delete pub;
}

void NetworkOutput3DWrapper::publishKeyframe(Frame* f)
{

	lsd_slam::keyframeMsg* fMsg = new lsd_slam::keyframeMsg();
...
void NetworkOutput3DWrapper::publishTrackedFrame(Frame* kf)
{
    //printf("At: %s:%d\n",__FUNCTION__, __LINE__);
	lsd_slam::keyframeMsg* fMsg = new lsd_slam::keyframeMsg();
;
	...
	memcpy(fMsg->camToWorld, kf->getScaledCamToWorld().cast<float>().data(), sizeof(float)*7);
    ...
    serializeMessageAndSend(pMsg); */

}


void NetworkOutput3DWrapper::publishKeyframeGraph(KeyFrameGraph* graph)
{
...
void NetworkOutput3DWrapper::publishTrajectory(std::vector<Eigen::Matrix<float, 3, 1>> trajectory, std::string identifier)
{
	// unimplemented ... do i need it?
}

void NetworkOutput3DWrapper::publishTrajectoryIncrement(Eigen::Matrix<float, 3, 1> pt, std::string identifier)
{
	// unimplemented ... do i need it?
}

void NetworkOutput3DWrapper::publishDebugInfo(Eigen::Matrix<float, 20, 1> data)
{
